import React from 'react';
import { Home, Bot, Sparkles, GraduationCap, Megaphone, Lock, Layout, X } from 'lucide-react';

const CATEGORIES = [
  { id: 'about', label: 'דף הבית', subtitle: 'אודות המערכת', icon: Home, color: 'slate' },
  { id: 'internal', label: 'מערכות ובוטים', subtitle: 'כלים פנימיים', icon: Bot, color: 'blue' },
  { id: 'tool', label: 'כלי בינה מלאכותית', subtitle: 'AI גנרטיבי', icon: Sparkles, color: 'cyan' },
  { id: 'training', label: 'הדרכות וסרטונים', subtitle: 'ספרייה מקצועית', icon: GraduationCap, color: 'amber' },
  { id: 'updates', label: 'עדכונים והודעות', subtitle: 'חדשות ואירועים', icon: Megaphone, color: 'purple' }
];

const Sidebar = ({ 
  activeTab, 
  setActiveTab, 
  darkMode, 
  sidebarOpen, 
  setSidebarOpen, 
  isAdmin, 
  setIsAdmin, 
  setIsLoginModalOpen 
}) => {

  const SidebarItem = ({ cat }) => {
    const isActive = activeTab === cat.id;
    const Icon = cat.icon;
    
    // Custom glowing color classes based on active state
    const activeColorClasses = darkMode ? {
      slate: 'bg-slate-500/20 text-slate-300 border-slate-500/30 shadow-[0_0_15px_rgba(148,163,184,0.15)]',
      blue: 'bg-blue-600 text-white shadow-[0_0_20px_rgba(37,99,235,0.3)] border-blue-500/30',
      cyan: 'bg-cyan-500 text-slate-950 shadow-[0_0_20px_rgba(6,182,212,0.4)] border-cyan-400/30 font-black',
      amber: 'bg-amber-500 text-slate-950 shadow-[0_0_20px_rgba(245,158,11,0.3)] border-amber-400/30 font-black',
      purple: 'bg-purple-600 text-white shadow-[0_0_20px_rgba(147,51,234,0.3)] border-purple-500/30'
    } : {
      slate: 'bg-slate-200 text-slate-800 border-slate-300/60 shadow-[0_4px_12px_rgba(148,163,184,0.1)]',
      blue: 'bg-blue-600 text-white shadow-[0_4px_15px_rgba(37,99,235,0.2)] border-blue-500/20',
      cyan: 'bg-cyan-600 text-white shadow-[0_4px_15px_rgba(8,145,178,0.2)] border-cyan-500/20 font-black',
      amber: 'bg-amber-500 text-slate-950 shadow-[0_4px_15px_rgba(245,158,11,0.15)] border-amber-500/25 font-black',
      purple: 'bg-purple-600 text-white shadow-[0_4px_15px_rgba(147,51,234,0.2)] border-purple-500/20'
    };

    return (
      <button 
        onClick={() => { setActiveTab(cat.id); setSidebarOpen(false); }}
        className={`w-full flex items-center gap-4 px-4 py-3.5 rounded-2xl border transition-all duration-300 group ${
          isActive 
            ? activeColorClasses[cat.color]
            : `${darkMode ? 'text-slate-400 bg-transparent border-transparent hover:bg-slate-800/40 hover:text-slate-200' : 'text-slate-500 bg-transparent border-transparent hover:bg-slate-100/60 hover:text-slate-800'}`
        }`}
      >
        <div className={`p-2 rounded-xl transition-all duration-300 ${
          isActive 
            ? 'bg-white/20' 
            : 'bg-slate-500/5 group-hover:bg-slate-500/10'
        }`}>
          <Icon size={20} />
        </div>
        <div className="text-right">
          <div className={`font-bold text-sm leading-tight transition-colors ${
            isActive ? '' : 'text-slate-700 dark:text-slate-200 group-hover:text-slate-950 dark:group-hover:text-white'
          }`}>{cat.label}</div>
          <div className={`text-[10px] font-medium mt-0.5 ${
            isActive ? 'opacity-80' : 'text-slate-400 dark:text-slate-500'
          }`}>{cat.subtitle}</div>
        </div>
      </button>
    );
  };

  return (
    <>
      {/* Mobile Sidebar Overlay */}
      {sidebarOpen && (
        <div 
          className="fixed inset-0 z-40 bg-black/60 lg:hidden backdrop-blur-md transition-opacity duration-300"
          onClick={() => setSidebarOpen(false)}
        ></div>
      )}

      {/* Sidebar Container */}
      <aside className={`fixed inset-y-0 right-0 w-72 z-50 transform transition-all duration-300 ease-out lg:translate-x-0 lg:static lg:block glass-panel ${
        darkMode ? 'bg-[#0b0f17]/95 border-l border-slate-900/50' : 'bg-white/95 border-l border-slate-200'
      } ${sidebarOpen ? 'translate-x-0 shadow-2xl' : 'translate-x-full rtl:-translate-x-full'}`}>
        
        <div className="h-full flex flex-col p-6 relative">
          {/* Mobile Close Button */}
          <button 
            onClick={() => setSidebarOpen(false)}
            className={`absolute top-4 left-4 p-2 rounded-xl lg:hidden transition-colors ${
              darkMode ? 'hover:bg-slate-800 text-slate-400 hover:text-slate-200' : 'hover:bg-slate-100 text-slate-500'
            }`}
            aria-label="סגור תפריט"
          >
            <X size={20} />
          </button>

          {/* Brand Logo Container */}
          <div className="flex flex-col items-center text-center mb-8 space-y-4 pt-6 lg:pt-2">
            <div className="relative w-32 h-32 mx-auto group cursor-pointer">
              {/* Outer decorative pulsing orb */}
              <div className="absolute -inset-1.5 bg-gradient-to-tr from-cyan-500/20 to-purple-500/20 rounded-[2.5rem] blur opacity-60 group-hover:opacity-100 group-hover:scale-105 transition-all duration-500"></div>
              
              {/* Main Container */}
              <div className="absolute inset-0 bg-slate-900/80 dark:bg-slate-950/90 rounded-[2.2rem] border border-cyan-500/25 flex items-center justify-center p-3 shadow-inner shadow-cyan-500/10">
                <div className="w-full h-full relative flex items-center justify-center overflow-hidden rounded-[1.6rem]">
                  <img 
                    src="/לוגו מדור מופ.png" 
                    alt="לוגו מדור מופ" 
                    className="w-full h-full object-contain filter drop-shadow-[0_0_8px_rgba(6,182,212,0.4)]" 
                    onError={(e) => { e.target.style.display='none'; }} 
                  />
                </div>
              </div>
            </div>

            <div className="space-y-1.5">
              <h1 className="font-black text-2xl tracking-tight bg-gradient-to-r from-cyan-400 via-blue-400 to-purple-400 bg-clip-text text-transparent">
                DIFS AI Hub
              </h1>
              <div className="inline-flex items-center gap-1.5 px-3 py-1 rounded-full bg-cyan-500/10 border border-cyan-500/20 shadow-[0_0_10px_rgba(6,182,212,0.05)]">
                <span className="w-1.5 h-1.5 rounded-full bg-cyan-400 animate-pulse"></span>
                <span className="text-[9px] font-black text-cyan-300 tracking-wider uppercase">מדור מחקר ופיתוח</span>
              </div>
            </div>
          </div>

          {/* Navigation Menu */}
          <nav className="flex-grow space-y-2.5 overflow-y-auto min-h-0 custom-scrollbar px-1">
            <div className="px-2 mb-2 text-[9px] font-black uppercase text-slate-400 dark:text-slate-500 tracking-widest">קטגוריות</div>
            {CATEGORIES.map(cat => <SidebarItem key={cat.id} cat={cat} />)}
          </nav>

          {/* Admin Toggle Panel */}
          <div className="mt-auto pt-6 border-t border-slate-200/10 dark:border-slate-800/60 shrink-0">
            <button 
              onClick={() => isAdmin ? setIsAdmin(false) : setIsLoginModalOpen(true)}
              className={`w-full flex items-center justify-center gap-2 py-3.5 rounded-2xl text-sm font-bold transition-all duration-300 border ${
                isAdmin 
                  ? 'bg-amber-500/20 text-amber-300 border-amber-500/30 shadow-[0_0_15px_rgba(245,158,11,0.15)]' 
                  : `${darkMode ? 'bg-slate-900/50 hover:bg-slate-800/80 text-slate-300 border-slate-800' : 'bg-slate-100 hover:bg-slate-200 text-slate-700 border-slate-200'}`
              }`}
            >
              {isAdmin ? <Layout size={16} /> : <Lock size={16} />}
              <span>{isAdmin ? 'יציאה מניהול' : 'כניסה לניהול'}</span>
            </button>
          </div>
        </div>
      </aside>
    </>
  );
};

export default Sidebar;
export { CATEGORIES };
